...
dist_ops = {"normal":lambda mu, scale: distributions.normal.Normal(loc=mu, scale=scale),
    ...
    if len(e) == 1:
        ...
        try:
            ...
            # Observe
                ...
                c = _totensor(x=c)
                if DEBUG:
                    print('Observe distribution: ', p)
                    print('Observe Value: ', c, "\n")

                try:
                    if "logW" in sig.keys():
                        sig["logW"] += p.log_prob(c)
                    else:
                        sig["logW"]  = p.log_prob(c)
                except:
                    if "logW" in sig.keys():
